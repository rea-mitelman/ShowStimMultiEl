function sessout = FindStimFiles( parentdir, monkey_name )

fdir = dir([parentdir '\' monkey_name(1) '*']);
fdir = sortdirs( fdir );
indx =1;
sessout = [];
for i=1:length(fdir),
    ...
    if isdir([parentdir '\' cursess '\info']),
        ...
        for j=1:length(dat.SESSparam.SubSess),
            ...
            [e_localstim,cnfig_localstim]=getfilenums_localstim(dat.SESSparam.SubSess(j));
            e=[e e_localstim];
            cnfig=[cnfig cnfig_localstim];
            
            for i=dat.SESSparam.SubSess(j).Files(1):dat.SESSparam.SubSess(j).Files(2)
                if dat.SESSparam.fileConfig(i).SCPStim
                    e=[e i];
                    cnfig=[];
                end
            end
