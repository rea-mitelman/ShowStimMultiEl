...
%
% NOTES         1. handling of matrix data is not perfect, pls use vector input
%               2. magnitude response is double the expected

% 13-jan-06 ES

function y = iirfilt(x,Fs,flp,fhp,N,band)
...
elseif nargin==3 | isempty(fhp), 
    [b a] = butter(2*N,flp/Fs*2);           % lowpass
elseif nargin>=4 & isempty(flp),
    wc = fhp/Fs*2;
    [blp alp] = butter(N,wc);               % lowpass
    [b a] = iirlp2hp(blp,alp,wc,wc);        % highpass
elseif nargin>=4 & band==1
    wc = [flp fhp]/Fs*2;
    [blp alp] = butter(N,wc(2));            % lowpass
    [b a] = iirlp2bp(blp,alp,wc(2),wc);     % bandpass
elseif nargin>=4
    wc = [flp fhp]/Fs*2;
    [blp alp] = butter(N,wc(2));            % lowpass
    [b a] = iirlp2bs(blp,alp,wc(2),wc);     % bandstop
end
